...
public class StreamingStatExtractorProgramStatistics {
	...
	private int getPlayCountFromProgramMap(
			Map<String, Map<String, Map<String, PlayCounter>>> programMap) {
		PlayCounter playCounter = new PlayCounter();
		if (programMap != null) {
			for (Iterator<Map<String, Map<String, PlayCounter>>> programMapIterator = programMap.values().iterator(); programMapIterator.hasNext(); ) {
	...
	private int getPlayCountFromProgramChannelMap(
			Map<String, Map<String, PlayCounter>> programChannelMap) {
		PlayCounter playCounter = new PlayCounter();
		if (programChannelMap != null) {
			for (Iterator<Map<String, PlayCounter>> channelProgramMapIterator = programChannelMap.values().iterator(); channelProgramMapIterator.hasNext(); ) {
	...
	private int getPlayCountFromProgramChannelDateMap(
			Map<String, PlayCounter> programChannelDateMap) {
		PlayCounter datePlayCounter = new PlayCounter(); 
		if (programChannelDateMap != null) {
			for (Iterator<PlayCounter> dateChannelProgramMapIterator = programChannelDateMap.values().iterator(); dateChannelProgramMapIterator.hasNext(); ) {
	...
	public String toString() {
		...
		for (Iterator<String> programIterator = titleMap.keySet().iterator(); programIterator.hasNext();) {
			...
			Map<String, Map<String, PlayCounter>> channelProgramMap = titleMap.get(programTitle);
			for (Iterator<String> channelIterator = channelProgramMap.keySet().iterator(); channelIterator.hasNext();) {
				String channel = channelIterator.next();
				sb.append("   - " + channel + "\n");
				Map<String, PlayCounter> programChannelDateMap = channelProgramMap.get(channel);
				for (Iterator<String> dateIterator = programChannelDateMap.keySet().iterator();dateIterator.hasNext(); ) {
